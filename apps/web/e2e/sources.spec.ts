...
test.describe('Brand sources', () => {
  test('Gibson page lists both manufacturer and reference sources with URLs', async ({ page }) => {
    ...
    await expect(
      page.getByRole('link', { name: /Gibson Serial Number Search/i }),
    ).toHaveAttribute('href', 'https://www.gibson.com/pages/serial-number-search');
    await expect(
      page.getByRole('link', { name: /Vintage Guitar Info Guy/i }),
    ).toHaveAttribute('href', 'https://guitarhq.com/gibson.html');
  });
  ...
  test('Fender page lists the three official support articles', async ({ page }) => {
    ...
    await expect(
      page.getByRole('link', { name: /American-made instrument/i }),
    ).toBeVisible();
    await expect(
      page.getByRole('link', { name: /Mexican-made instrument/i }),
    ).toBeVisible();
    await expect(
      page.getByRole('link', { name: /Japanese-made instrument/i }),
    ).toBeVisible();
  });
});
